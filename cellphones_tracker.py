...
<button
           id="downloadCsvBtn"
           style="display: inline-block; padding: 12px 25px; background-color: #0366d6; color: white; text-decoration: none; border-radius: 6px; font-size: 1em; font-weight: 500; box-shadow: 0 2px 5px rgba(0,0,0,0.15); transition: background-color 0.2s ease, transform 0.2s ease; border: none; cursor: pointer;"
    ...
    <p style="font-size: 0.9em; color: #555; margin-top: 10px;">
        (Bạn có thể tải xuống dữ liệu để phân tích hoặc chạy cập nhật để lấy dữ liệu mới nhất)
    </p>
</div>
    ...
    html_content += """
        ...
        <script>
            ...
            // Hàm tải xuống file CSV từ GitHub
document.getElementById('downloadCsvBtn').addEventListener('click', function() {
    const owner = 'Trung051';
    const repo = 'Toolchekgia';
    const path = 'tat_ca_san_pham_da_quet.csv';
    ...
    fetch(rawUrl)
        ...
        .then(csvContent => {
            // Thêm BOM (Byte Order Mark) để Excel nhận diện UTF-8 đúng
            const BOM = '\uFEFF';
            ...
            link.href = url;
            link.setAttribute('download', 'tat_ca_san_pham_da_quet.csv');
